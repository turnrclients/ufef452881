...
// for logo change
if ($(originalEl).hasClass('site-logo-img')) {

    const container = $(originalEl).parent();

    container.find('.generated-logo').hide(); // hide text logo
    $(originalEl).show();                     // show image logo
}
             //  TRACK IMAGE FILE
...
// });
    ...
    function toggleEditableClasses(enable) {
        ...
        if (enable) {
            ...
            wrapper.find('.generated-logo').addClass('updateTextLogo');   //New code
...
$(document).on('click', '.category-sub a', function (e) {
    ...
    let uploadPromises = [];
    let imageElements = []; // store img elements for later src refresh
        ...
        $(this).find('img').each(function () {
            ...
            const promise = new Promise((resolve) => {
                ...
                urlToFile(imgSrc, getFileNameFromImgSrc(imgEl), function (file) {
                    ...
                    formData.append('imgFiles', file); // SAME KEY
                    formData.append('imgFileNames', getFileNameFromImgSrc(imgEl));

                    imageElements.push(imgEl); // save for later refresh
